...
export class AccountWorker {
  ...
  async handleOrder(order) {
    const { orderId, acceptUrl, declineUrl, source, address, zip, state, forwardingEmail } = order;
    this.stats.detected++;
    ...
    if (!region.allowed && !region.decided) {
      ...
      this.log.warn('order region undetermined — not acting', { orderId, source, meta: region.meta });
      return this._report({ ...meta, action: 'skip', accepted: false, declined: false, reason: 'region_unknown' });
    }
    ...
    // Enrich with location/identity for the feed.
    event = {
      ...event,
