...
- **Order events store** ([src/store/orderEventsStore.js](src/store/orderEventsStore.js))
  is an append-only JSONL log written on every detected order (accept/decline/skip/
  unattributed). It powers `GET /api/orders` (the dashboard's live feed) and
  `GET /api/stats` (per-account rolling 24h/7d/30d counts). Reads parse the whole file
  in memory — fine at order-trickle volume, not meant to scale to a real event store.
- **Gmail OAuth/connect helpers** ([src/gmail/oauth.js](src/gmail/oauth.js),
  [src/gmail/connect.js](src/gmail/connect.js)) build the consent URL, exchange the
  auth code, and wire a refresh token into the single
  [GmailConnectionStore](src/store/gmailConnection.js) plus the Pub/Sub `users.watch()`
  registration. Watch registration failing (e.g. topic not configured) does **not**
  fail the connect — the token is still stored and detection falls back to polling.
...
and [test/gmail.test.js](test/gmail.test.js); orchestration/end-to-end coverage lives in
[test/integration.test.js](test/integration.test.js), [test/multiUser.test.js](test/multiUser.test.js),
and [test/system.test.js](test/system.test.js) (94 tests total — re-run `npm test` after
changes rather than trusting this count, it drifts).
